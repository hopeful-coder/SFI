...
# #Load in new data
pop <- read.csv('UCLAQuarterlyReportMay2020Pop.csv', skip = 1)
arrests <- read.csv('UCLAQuarterlyReportMay2020Arrests.csv')
# 
...
names(ncc)[1] = 'id'
names(ncc)[4] = 'ncc_reason'
names(ncc)[9] = 'ncc_date'
ncc = ncc[, c('id', 'ncc_date', 'ncc_reason')]
ncc = ncc[ncc$ncc_reason == 'NCC - Non-Conforming Referral',]
...
for(i in 1:length(unique.ids)){
  ...
  if(!is.na(words$Arrest.Date)){
  words$Arrest.Date = as.Date(words$Arrest.Date, format = '%Y-%m-%d')}
  ...
  if(!is.na(arrests.abbie)){
    ...
    abbie$num.arrests10 = sum(arrest_quarters == 10, na.rm = T)

    arrests.abbie = words$Arrest.Date
    arrests.abbie = sort(arrests.abbie)
    
    if(!is.na(arrests.abbie)){
    post_one = if_else(words$Arrest.Date <= words$X.5 + 365, 1, 0)
    arrests.abbie = arrests.abbie[post_one == 1]
    abbie$num.arrests = length(arrests.abbie)
    arrests.type = arrests.type[post_one == 1]
    arrest_quarters = arrest_quarters[post_one == 1]
...
# 
  ...
  abbie$ever.arrested = ifelse(length(arrests.abbie) == 0, 0,
                               ifelse(is.na(arrests.abbie), 0, 1))
...
pfs <- (pfs[as.Date(pfs$Intake.Date, format = '%m/%d/%Y') <= as.Date('05/31/2020', format = '%m/%d/%Y'),])
# pfs = pfs[is.na(pfs$ncc),]

other <- pas              

rashmi1 = rbind(pfs, other)
remove.names = c('aq1', 'aq2', 'aq3', 'aq4', 'aq5', 'aq6', 'aq7', 'aq8', 'aq9',
                 'num.arrests1', 'num.arrests2', 'num.arrests3', 'num.arrests4',
                 'num.arrests5', 'num.arrests6', 'num.arrests7', 'num.arrests8',
                 'num.arrests9', 'num.arrests10',
                 'arrest1type', 'arrest2type', 'arrest3type', 'arrest4type', 'arrest5type', 'arrest6type', 'arrest7type', 'arrest8type')
remove.names = which(names(rashmi1) %in% remove.names)
rashmi1 = rashmi1[, -remove.names]

write.csv(rashmi1, 'SFI_Compare.csv')
...
comb_again_age = paste0(round(mean(comb_again$age), 1), ' (', round(sd(comb_again$age), 1), ')')

M <- as.table(rbind(as.numeric(pfs_risk.level[,2]), as.numeric(other_risk.level[,2])))
risk_p = chisq.test(M)$p.value

demo_df = data.frame(variable = c('# of Sample Population Members', 'Age, Mean (SD)', 'Male', 'Female', 'Race', 'Latino/Hispanic', 'White', 'Black', 'Other'),
                     pfs = c(npfs, pfs_age, paste0(round(prop.table(table(pfs$gender))[2]*100, 1), '%'), paste0(round(prop.table(table(pfs$gender))[1]*100, 1), '%'), '',  paste0(round(prop.table(table(pfs$race))[4]*100, 1), '%'), paste0(round(prop.table(table(pfs$race))[3]*100, 1), '%'), paste0(round(prop.table(table(pfs$race))[1]*100, 1), '%'), paste0(round(prop.table(table(pfs$race))[2]*100, 1), '%')),
                     other = c(nother, other_age, paste0(round(prop.table(table(other$gender))[2]*100, 1), '%'), paste0(round(prop.table(table(other$gender))[1]*100, 1), '%'), '',  paste0(round(prop.table(table(other$race))[4]*100, 1), '%'), paste0(round(prop.table(table(other$race))[3]*100, 1), '%'), paste0(round(prop.table(table(other$race))[1]*100, 1), '%'), paste0(round(prop.table(table(other$race))[2]*100, 1), '%')),
                     pval = c('-', round(age_p, 4), round(gender_p, 4), '-', round(race_p, 4), '-', '-', '-', '-'),
                     overall = c(nrow(comb_again), comb_again_age, paste0(round(prop.table(table(comb_again$gender))[2]*100, 1), '%'), paste0(round(prop.table(table(comb_again$gender))[1]*100, 1), '%'), '',  paste0(round(prop.table(table(comb_again$race))[4]*100, 1), '%'), paste0(round(prop.table(table(comb_again$race))[3]*100, 1), '%'), paste0(round(prop.table(table(comb_again$race))[1]*100, 1), '%'), paste0(round(prop.table(table(comb_again$race))[2]*100, 1), '%')
))
kable(demo_df)



tab1 = data.frame(variable = c('# of Sample Population Members', 'Aggregate Number of Arrests', '# of Members Arrests 1+ times in 12 months', 'Mean Number of Arrests', 'Rate of Re-Arrest', 'Relative Reduction in Mean Number of Arrests', 'Relative Reduction in Rate of Re-Arrest'),
                  'Control Group' = c(nother, sum(other$num.arrests), sum(other$ever.arrested), sum(other$num.arrests)/nother, sum(other$ever.arrested)/nother, (sum(other$num.arrests)/nother - sum(pfs$num.arrests)/npfs)/(sum(other$num.arrests)/nother), 2),
                  'Service Group' = c(npfs, sum(pfs$num.arrests), sum(pfs$ever.arrested), sum(pfs$num.arrests)/npfs, sum(pfs$ever.arrested)/npfs, 2, (sum(other$ever.arrested)/nother - sum(pfs$ever.arrested)/npfs)/(sum(other$ever.arrested)/nother)))
                    
                
kable(tab1)

ncc2 = pfs[!is.na(pfs$ncc), ]
reg = pfs[is.na(pfs$ncc),]

person.time = rowSums(!is.na(pfs[,42:51]))

```

```{r}
pfs.risk = prop.table(table(pfs$risk.level))
other.risk = prop.table(table(other$risk.level))
risk.df = data.frame(risk.level = c('Low', 'Low/Moderate', 'Moderate', 'High', 'Very High', 'Undeclared'),
                     'Service Group'        = c(pfs.risk[3], pfs.risk[4], pfs.risk[5], pfs.risk[2], pfs.risk[6], pfs.risk[1]),
                     'Control Group'        = c(other.risk[3], other.risk[4], other.risk[5], other.risk[2], other.risk[6], other.risk[1]))
risk.df = melt(risk.df, by = 'risk.level')
risk.df$risk.level = factor(risk.df$risk.level, levels = c('Low', 'Low/Moderate', 'Moderate', 'High', 'Very High', 'Undeclared'))
names(risk.df)[2] = 'Group'
ggplot(data=risk.df, aes(x=risk.level, y=value, fill=Group))+
  geom_bar(stat="identity", position=position_dodge())+
  geom_text(aes(label=paste0(round(value*100, 1), '%')), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+
  scale_fill_brewer(palette="Paired")+
  labs(x = 'Risk Level', y = 'Percentage (%) of Group Population', title = 'Figure 1: ORAS Risk Level at Intake')+
  theme_minimal()+
...
```{r eval = F, echo=FALSE, message=FALSE, warning=FALSE, cache=T}

pfs_pop = colSums(pfs[,c(20:29)])
pfs_clean = colSums(pfs[,c(32:41)])
pfs_a = pfs_pop - pfs_clean
pfs_a <- data.frame(index = c(1:10),
                    arrests = pfs_a)
pfs_pop = data.frame(index = c(1:10),
                  pop = pfs_pop)
# ggplot() + geom_line(aes(x = index, y = arrests), data = pfs_a) + geom_line(aes(x = index, y = pop), data = pfs_pop)
other_pop = colSums(other[,c(20:29)])
other_clean = colSums(other[,c(32:41)])
other_a = other_pop - other_clean
other_a <- data.frame(index = c(1:10),
                    arrests = other_a)
other_pop = data.frame(index = c(1:10),
                     pop = other_pop)
...
pfs$ncc2 = ifelse(is.na(pfs$ncc), 0, 1)

final$group = 'pfs'
final2$group = 'other'
final$num.arrests = colSums(pfs[,c(53:62)], na.rm = T)
final2$num.arrests = colSums(other[,c(53:62)], na.rm = T)
...
pfs$a10 = paste0(pfs$aq10, pfs$arrest10type)
...
other$a10 = paste0(other$aq10, other$arrest10type)
...
pls.f = pls[c(2,4,6,8,11,13,15,17,19,1),]
pls.m = pls[c(3,5,7,9,12,14,16,18,20,1),]

pls2.f = pls2[c(2,4,6,8,10,12,14,16,19,1),]
pls2.m = pls2[c(3,5,7,9,11,13,15,17,20,1),]
...
pop.df = data.frame(quarter = c(1:10),
                       'Service Group' = pfs_pop[,2],
                       'Control Group' = other_pop[,2])
pop.df = melt(pop.df, id.vars = 'quarter')
arrest.df = data.frame(quarter = c(1:10),
                       'Service Group' = pfs_a[,2],
                       'Control Group' = other_a[,2])
arrest.df = melt(arrest.df, id.vars = 'quarter')

names(pop.df)[2] = 'Group'
names(arrest.df)[2] = 'Group'
...
ggplot(data=pop.df, aes(x=quarter, y=value, color=Group)) +
  geom_line(stat="identity", position=position_dodge(), size = 4)+
  geom_line(data=arrest.df, aes(x=quarter, y=value, color=Group), size = 3)+
  scale_fill_brewer(palette="Paired")+
  labs(x = 'Quarter', y = 'Percentage (%) of Group Population')+
  theme_minimal()

ggplot(data=pop.df, aes(x=quarter, y=value, color=Group)) +
   geom_line(stat="identity", position=position_dodge(), size = 4)+
   geom_line(data=arrest.df, aes(x=quarter, y=value, color=Group), linetype = 'dashed', size = 3)+
   scale_fill_brewer(palette="Paired")+
   labs(x = 'Quarter', y = 'Total Population/Arrests', title = 'Supplemental Figure: Population and Arrests of Cohort 1 Per Quarter')+
   theme_minimal()+scale_x_continuous("Quarter", labels = as.character(pop.df$quarter), breaks = pop.df$quarter)

ggplot(data=final3, aes(x=index, y=prop, color=group)) +
   geom_line(stat="identity", position=position_dodge(), size = 2)+
   geom_line(data=arrest.df, aes(x=quarter, y=value, color=Group), linetype = 'dashed', size = 3)+
   scale_fill_brewer(palette="Paired")+
   labs(x = 'Quarter', y = 'Total Population/Arrests', title = 'Supplemental Figure: Population and Arrests of Cohort 1 Per Quarter')+
   theme_minimal()+scale_x_continuous("Quarter", labels = as.character(pop.df$quarter), breaks = pop.df$quarter)
...
services = data.frame(services = c('Case Management', 'MRT', "Triple P", "Seek Safety", 'Job Readiness', 'Other CBT'))
services$prop = 0
supplemental$anycbt = ifelse(supplemental$MRT == 1|supplemental$Case_Management == 1 |
                               supplemental$Triple_P == 1|
                               supplemental$Seek_Safety == 1|
                               supplemental$JobReadiness == 1 |
                               supplemental$Other_CBT == 1, 1, 0)
for(i in 7:12){
...
services2$ever = 0
for(i in 100:105){
    services2$never[i-99] = round(prop.table(table(n.a[,i], useNA = 'always'))[2]*100, 1)
    services2$ever[i-99]  = round(prop.table(table(e.a[,i], useNA = 'always'))[2]*100, 1)
}
# for(i in 130:146){
...
bigone$MRT_Step_Achieved = as.numeric(as.character(bigone$MRT_Step_Achieved))
prop.table(table(bigone$Case_Management_Progress, bigone$ever.arrested), 2)
prop.table(table(bigone$MRT, bigone$ever.arrested),2)
tapply(as.numeric(as.character(bigone$MRT_Step_Achieved)), bigone$ever.arrested, mean, na.rm = T)
tapply(as.numeric(as.character(bigone$MRT_Step_Achieved)), bigone$ever.arrested, sd, na.rm = T)
mean(bigone$MRT_Step_Achieved)
sd(bigone$MRT_Step_Achieved)

prop.table(table(bigone$JobReadiness, bigone$ever.arrested),2)
prop.table(table(bigone$Triple_P, bigone$ever.arrested),2)
prop.table(table(bigone$Seek_Safety, bigone$ever.arrested),2)


bigone$Triple_P_Modules_completed = as.numeric(as.character(bigone$Triple_P_Modules_completed))
tapply(as.numeric(as.character(bigone$Triple_P_Modules_completed)), bigone$ever.arrested, mean, na.rm = T)
tapply(as.numeric(as.character(bigone$Triple_P_Modules_completed)), bigone$ever.arrested, sd, na.rm = T)
mean(bigone$Triple_P_Modules_completed, na.rm = T)
sd(bigone$Triple_P_Modules_completed, na.rm = T)

bigone$JR_Step_Achieved = as.numeric(as.character(bigone$JR_Step_Achieved))
tapply(as.numeric(as.character(bigone$JR_Step_Achieved)), bigone$ever.arrested, mean, na.rm = T)
tapply(as.numeric(as.character(bigone$JR_Step_Achieved)), bigone$ever.arrested, sd, na.rm = T)
mean(bigone$JR_Step_Achieved, na.rm = T)
sd(bigone$JR_Step_Achieved, na.rm = T)


bigone$SeekSafety_Modules_completed = as.numeric(as.character(bigone$SeekSafety_Modules_completed))
tapply(as.numeric(as.character(bigone$SeekSafety_Modules_completed)), bigone$ever.arrested, mean, na.rm = T)
tapply(as.numeric(as.character(bigone$SeekSafety_Modules_completed)), bigone$ever.arrested, sd, na.rm = T)
mean(bigone$SeekSafety_Modules_completed, na.rm = T)
sd(bigone$SeekSafety_Modules_completed, na.rm = T)
